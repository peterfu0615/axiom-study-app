...
允许的样式只有 axiomLine、axiomAuxiliary、axiomPoint、axiomLabel、axiomAngle、axiomRightAngle、axiomAxis，以及兼容样式 thick、dashed、->。标签必须简短、可读，不包含命令。
...
export async function renderGeneratedTikzWithRepair<T extends {
  ...
  for (let repairCount = 0; repairCount <= TIKZ_REPAIR_LIMIT; repairCount += 1) {
    ...
    const accepted = result.renderStatus === 'rendered' && result.validationStatus !== 'rejected'
    if (accepted || repairCount === TIKZ_REPAIR_LIMIT) {
      return { source, result, repairCount }
    }
    const repaired = await input.repair(buildTikzRepairPrompt(
      source,
      result.errorCode || 'render_failed',
      result.validationErrors?.join('；') || result.errorMessage || '图形渲染失败',
    ), repairCount + 1)
