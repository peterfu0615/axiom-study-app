...
function previewStructureNodes(state: string) {
  if (!state.startsWith('structure-long') && state !== 'structure-820x620') return nodes
  ...
  const base = state === 'structure-long-detail' || state === 'structure-820x620'
    ...
    : nodes
  const extra = Array.from({ length: 12 }, (_, chapterIndex) => {
    ...
    const children = Array.from({ length: 5 }, (_, sectionIndex) => {
      const sectionNumber = sectionIndex + 1
      const sectionId = `${chapterId}-section-${sectionNumber}`
      return {
        id: sectionId, textbook_id: 'math-book', subject: '数学', canonical_name: `${chapterNumber}.${sectionNumber} 综合方法`, node_type: 'section', parent_id: chapterId,
        path: `${chapter.path}/${chapterNumber}.${sectionNumber} 综合方法`, sort_order: sectionIndex, source_page_start: chapter.source_page_start + sectionIndex,
        source_page_end: chapter.source_page_start + sectionIndex + 2, evidence_text: `章节依据：${chapterNumber}.${sectionNumber} 综合方法`, source_path: '/preview/math.pdf',
        extraction_method: 'pdf_text', confidence: .86, verification_status: 'needs_review', merged_into_id: null, archived_at: null, created_at: now, updated_at: now,
      }
    })
  ...
  const status = state === 'import-failed' || state === 'global-analysis-failed'
    ...
    : state === 'global-analysis-tags' || state === 'import-progress-minimal'
      ...
      : state === 'global-analysis-audit'
        ? 'ai_auditing'
        : state === 'import-processing' || state === 'global-analysis-structure'
