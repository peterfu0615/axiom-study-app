...
function CurriculumPreviewShell({ state }: { state: string }) {
  ...
  const workspace = section === 'curriculum'
    ? <CurriculumWorkspace initialView={state.startsWith('tags-') || state.startsWith('relabel-') || state === 'tag-table-centered' || state === 'custom-select-open' ? 'tags' : state.startsWith('review-') || state === 'bulk-review-success' ? 'review' : 'structure'} />
    : section === 'capture'
