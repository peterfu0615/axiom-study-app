...
//!   1. 全新库一路跑到 39，且与 sqlx Migrator 校验兼容（幂等重跑）；
//!   2. 27 状态的库可以升级到 39；
...
mod tests {
    ...
    /// 迁移列表完整性：版本必须恰好为 1..=39 且严格递增。
    /// 用户真实库已应用 codex 分支的 24–27，列表缺号会让任何校验拒绝启动。
    #[test]
    fn migration_list_covers_versions_1_through_39_exactly() {
        let versions: Vec<i64> = axiom_migrations()
            .iter()
            .map(|migration| migration.version)
            .collect();
        let expected: Vec<i64> = (1..=39).collect();
        assert_eq!(versions, expected, "迁移列表必须严格等于 1..=39");
    }
    ...
    fn diagram_source_and_rendered_asset_constraints_survive_restart() {
        tauri::async_runtime::block_on(async {
            ...
            let mut conn = connect(&temp).await;
            migrate_embedded_schema(&mut conn, &migrations_up_to(39))
                .await
            ...
            migrate_embedded_schema(&mut reopened, &migrations_up_to(39))
                .await
                .expect("diagram migration must be restart-safe");
            assert_eq!(max_applied_version(&mut reopened).await, 39);
        });
    }
    ...
    #[test]
    fn database_at_version_27_upgrades_to_39() {
        tauri::async_runtime::block_on(async {
            ...
            migrate_embedded_schema(&mut conn, &migrations_up_to(39))
                .await
                .expect("0028–0039 必须能在 0027 状态库上成功应用");
            assert_eq!(max_applied_version(&mut conn).await, 39);
    ...
    #[test]
    fn real_user_database_copy_upgrades_to_version_39() {
        ...
        tauri::async_runtime::block_on(async {
            ...
            migrate_embedded_schema(&mut conn, &migrations_up_to(39))
                .await
                .expect("用户库副本必须通过 checksum 校验并成功升级到 39");
            assert_eq!(max_applied_version(&mut conn).await, 39);
