...
//!   1. 全新库一路跑到 35，且与 sqlx Migrator 校验兼容（幂等重跑）；
//!   2. 27 状态的库可以升级到 35；
...
mod tests {
    ...
    /// 迁移列表完整性：版本必须恰好为 1..=35 且严格递增。
    /// 用户真实库已应用 codex 分支的 24–27，列表缺号会让任何校验拒绝启动。
    #[test]
    fn migration_list_covers_versions_1_through_35_exactly() {
        let versions: Vec<i64> = axiom_migrations()
            .iter()
            .map(|migration| migration.version)
            .collect();
        let expected: Vec<i64> = (1..=35).collect();
        assert_eq!(versions, expected, "迁移列表必须严格等于 1..=35");
    }
    ...
    fn today_engine_persistence_is_idempotent_and_survives_soft_delete() {
        tauri::async_runtime::block_on(async {
            ...
            let mut conn = connect(&temp).await;
            migrate_embedded_schema(&mut conn, &migrations_up_to(35))
                .await
    ...
    #[test]
    fn database_at_version_27_upgrades_to_35() {
        tauri::async_runtime::block_on(async {
            ...
            migrate_embedded_schema(&mut conn, &migrations_up_to(35))
                .await
                .expect("0028–0035 必须能在 0027 状态库上成功应用");
            assert_eq!(max_applied_version(&mut conn).await, 35);
    ...
    #[test]
    fn real_user_database_copy_upgrades_to_version_35() {
        ...
        tauri::async_runtime::block_on(async {
            ...
            migrate_embedded_schema(&mut conn, &migrations_up_to(35))
                .await
                .expect("用户库副本必须通过 checksum 校验并成功升级到 35");
            assert_eq!(max_applied_version(&mut conn).await, 35);
