...
//!   1. 全新库一路跑到 30，且与 sqlx Migrator 校验兼容（幂等重跑）；
//!   2. 27 状态的库可以升级到 30；
...
mod tests {
    ...
    /// 全新库必须能一路跑到 30（含 codex 原文的 24–27 与 0028–0030 的衔接）。
    /// 随后用与 sqlx Migrator 完全一致的校验逻辑重跑两遍：
    ///   - embedded runner 幂等（全部已应用，不再执行任何脚本）；
    ///   - sqlx Migrator（plugin 的同款路径）校验 checksum 全部通过且不应用。
    #[test]
    fn fresh_database_reaches_30_and_stays_sqlx_compatible() {
        tauri::async_runtime::block_on(async {
            let temp = TempDb::new("fresh");
            let mut conn = connect(&temp).await;
            let migrations = migrations_up_to(30);
            migrate_embedded_schema(&mut conn, &migrations)
                .await
                .expect("全新库必须能完整迁移到 30（裸 BEGIN 由 runner 剥离）");
            assert_eq!(max_applied_version(&mut conn).await, 30);
    ...
    /// 迁移列表完整性：版本必须恰好为 1..=30 且严格递增。
    /// 用户真实库已应用 codex 分支的 24–27，列表缺号会让任何校验拒绝启动。
    #[test]
    fn migration_list_covers_versions_1_through_30_exactly() {
        let versions: Vec<i64> = axiom_migrations()
            .iter()
            .map(|migration| migration.version)
            .collect();
        let expected: Vec<i64> = (1..=30).collect();
        assert_eq!(versions, expected, "迁移列表必须严格等于 1..=30");
    }
    ...
    fn sibling_guard_cleans_duplicates_reparents_children_and_replays() {
        tauri::async_runtime::block_on(async {
            ...
            migrate_embedded_schema(&mut conn, &migrations_up_to(30))
                .await
                .expect("0026–0030 必须能在含脏数据的库上成功应用");
    ...
    #[test]
    fn database_at_version_27_upgrades_to_30() {
        tauri::async_runtime::block_on(async {
            ...
            migrate_embedded_schema(&mut conn, &migrations_up_to(30))
                .await
                .expect("0028–0030 必须能在 0027 状态库上成功应用");
            assert_eq!(max_applied_version(&mut conn).await, 30);
    ...
    #[test]
    fn real_user_database_copy_upgrades_to_version_30() {
        ...
        tauri::async_runtime::block_on(async {
            ...
            migrate_embedded_schema(&mut conn, &migrations_up_to(30))
                .await
                .expect("用户库副本必须通过 checksum 校验并成功升级到 30");
            assert_eq!(max_applied_version(&mut conn).await, 30);
    ...
    fn failed_textbook_pages_rebuild_preserves_rows_and_accepts_failed() {
        tauri::async_runtime::block_on(async {
            ...
            migrate_embedded_schema(&mut conn, &migrations_up_to(30))
                .await
                .expect("0026–0030 必须成功应用（含 0029 表重建）");
