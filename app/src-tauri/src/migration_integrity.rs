...
//!   1. 全新库一路跑到 54，且与 sqlx Migrator 校验兼容（幂等重跑）；
//!   2. 27 状态的库可以升级到 54；
...
mod tests {
    ...
    /// 迁移列表完整性：版本必须恰好为 1..=54 且严格递增。
    /// 用户真实库已应用 codex 分支的 24–27，列表缺号会让任何校验拒绝启动。
    #[test]
    fn migration_list_covers_versions_1_through_54_exactly() {
        let versions: Vec<i64> = axiom_migrations()
            .iter()
            .map(|migration| migration.version)
            .collect();
        let expected: Vec<i64> = (1..=54).collect();
        assert_eq!(versions, expected, "迁移列表必须严格等于 1..=54");
    }
    ...
    fn problem_library_profiles_and_duplicate_decisions_are_bounded() {
        tauri::async_runtime::block_on(async {
            ...
            let mut conn = connect(&temp).await;
            migrate_embedded_schema(&mut conn, &migrations_up_to(54))
                .await
